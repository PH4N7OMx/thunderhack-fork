...
public class ElytraSwap extends Module {
    ...
    private boolean isOnGroundLastTick = true;
    ...
    public void onEnable() {
        if (mode.getValue() == Mode.Enable) swapChest(true);
    }

    @Override
    public void onUpdate() {
        if (mode.getValue() != Mode.Bind || mc.currentScreen != null) return;

        boolean onGroundNow = mc.player.isOnGround();

        if (onlyGround.getValue() && !onGroundNow) {
            if (switchButton.getValue().getKey() != -1 && isKeyPressed(switchButton.getValue().getKey())) {
                queuedSwap = true;
            }
        } else {
            if (!isOnGroundLastTick && onGroundNow && queuedSwap) {
                swapChest(false);
                queuedSwap = false;
            }
        ...
        isOnGroundLastTick = onGroundNow;
    }
    ...
    public static int getChestPlateSlot() {
        ...
        for (Item item : items) {
            ...
            if (slot.found()) return slot.slot();
        }
        return -1;
    }
