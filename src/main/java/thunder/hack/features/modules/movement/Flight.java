...
public class Flight extends Module {
    ...
    private final Setting<Boolean> BoatOnly = new Setting<>("BoatOnly", false);
    ...
    public void onEventSync(EventSync event) {
        if (BoatOnly.getValue() && !isBoatNear(1.0)) {
            return;
        }
    ...
    private boolean isBoatNear(double radius) {
        if (mc.player == null || mc.world == null) return false;

        Box box = mc.player.getBoundingBox().expand(radius, radius, radius);
