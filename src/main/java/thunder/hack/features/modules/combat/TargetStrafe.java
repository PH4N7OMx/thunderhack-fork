...
public class TargetStrafe extends Module {
    ...
    public Setting<Float> distance = new Setting<>("Distance", 1.3F, 0.1F, 7f);

    private final Setting<Boost> boost = new Setting<>("Boost", Boost.None);
    public Setting<Float> setAirSpeed = new Setting<>("AirSpeed", 1.3F, 0.0F, 2f, v -> boost.getValue() == Boost.None);
    public Setting<Float> setGrSpeed = new Setting<>("GrSpeed", 1.3F, 0.0F, 2f, v -> boost.getValue() == Boost.None);
    private final Setting<Boolean> rotFix = new Setting<>("Predictive", false);
    ...
    public double calculateSpeed(EventMove move) {
        ...
        if (mc.player.isOnGround()) {
            ...
            float groundSpeed = boost.getValue() == Boost.None ? setGrSpeed.getValue() : 1.3F;
            n8 = groundSpeed * n7;
            if (move.getY() > 0) {
            ...
            float airSpeed = boost.getValue() == Boost.None ? setAirSpeed.getValue() : 1.3F;
            n8 = airSpeed * n7;
        }
    ...
    public void onMove(EventMove event) {
        ...
        if (canStrafe()) {
            if (Aura.target != null && ModuleManager.aura.isEnabled()) {
                ...
                double wrap;
                if (rotFix.getValue()) {
                    float targetYaw = Aura.target.getYaw();
                    float targetPitch = Aura.target.getPitch();

                    float yaw = targetYaw * (float) Math.PI / 180;
                    float pitch = targetPitch * (float) Math.PI / 180;
                    ...
                    wrap = Math.atan2(directionZ, directionX);
                } else {
                    wrap = Math.atan2(mc.player.getZ() - Aura.target.getZ(), mc.player.getX() - Aura.target.getX());
